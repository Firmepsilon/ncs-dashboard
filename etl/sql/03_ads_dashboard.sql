...
CREATE EXTERNAL TABLE ncs_ads.ads_area_cost (
  station_area STRING, total_kwh DOUBLE, revenue DOUBLE, cost DOUBLE, profit DOUBLE, profit_rate DOUBLE
)
...
SELECT
    ...
    total_kwh,
    revenue,
    ROUND(total_kwh*0.6,2)              AS cost,
    ROUND(revenue-total_kwh*0.6,2)      AS profit,
    ROUND((revenue-total_kwh*0.6)/NULLIF(revenue,0)*100,2) AS profit_rate
FROM (
  SELECT station_area, total_kwh, total_fee AS revenue
  FROM ncs_dws.dws_area_agg
) t;
...
-- ---------------------------------------------------------------------
-- 11. ads_hour_area：24小时×区域充电分布（堆叠柱状图用）
-- ---------------------------------------------------------------------
DROP TABLE IF EXISTS ncs_ads.ads_hour_area;
CREATE EXTERNAL TABLE ncs_ads.ads_hour_area (
  hour INT,
  station_area STRING,
  sessions BIGINT,
  total_kwh DOUBLE
...
STORED AS PARQUET
LOCATION '/user/hive/warehouse/ncs_ads.db/ads_hour_area';

INSERT OVERWRITE TABLE ncs_ads.ads_hour_area
SELECT hour, station_area, sessions, total_kwh
FROM ncs_dws.dws_hour_area_agg;
...
UNION ALL SELECT 'ads_hour_area',        COUNT(*) FROM ncs_ads.ads_hour_area;
